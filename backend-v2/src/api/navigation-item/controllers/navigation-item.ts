...
import { resolveAllowedNavigationIds } from '../../../services/tenant-visible-ids';

async function resolveTenantOwnedNavigationIds(
  strapi: any,
...
export default factories.createCoreController('api::navigation-item.navigation-item', ({ strapi }) => ({
  async find(ctx: Context) {
    ...
    const linkedIds = await resolveAllowedNavigationIds(strapi, access.visibleContentItemIds);
    const tenantOwnedIds = await resolveTenantOwnedNavigationIds(strapi, access.tenantId);
    ...
    const allItems = await strapi.entityService.findMany('api::navigation-item.navigation-item', {
      filters,
      sort: sortObj,
      limit: fetchLimit,
  ...
  async findOne(ctx: Context) {
    ...
    const item = await strapi.entityService.findOne('api::navigation-item.navigation-item', id, {
      populate: (ctx.query as any)?.populate,
    });
